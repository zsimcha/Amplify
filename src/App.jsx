...
const App = () => {
  ...
  // Tone-mapped colors for light backgrounds (Mobile Cards, Modals, Desktop Cards)
  const getTierColor = (tier) => {
    if (tier === 'silver') return 'text-slate-500';
    if (tier === 'gold') return 'text-amber-600';
    if (tier === 'diamond') return 'text-indigo-600';
    return 'text-slate-900';
  };

  // Subtle, muted glows to prevent the cards from looking "loud"
  const getTierBgGlow = (tier) => {
    if (tier === 'silver') return 'from-slate-50';
    if (tier === 'gold') return 'from-amber-50';
    if (tier === 'diamond') return 'from-indigo-50';
  ...
  const renderTierCardContent = (tier) => (
    <div className="flex flex-col h-full relative z-10">
        <div className="flex justify-between items-center mb-5 md:mb-6">
            <h3 className={`text-xl md:text-2xl font-black uppercase tracking-tighter ${getTierColor(tier)}`}>{tier}</h3>
        ...
        <div className="text-center py-6 md:py-8 bg-gradient-to-b from-slate-50 to-white rounded-2xl border border-slate-100 mb-5 md:mb-6 shadow-inner">
            ...
            <p className={`text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter ${getTierColor(tier)} drop-shadow-sm`}>{appData.tierData[tier].prize}</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 md:gap-4 mb-5 md:mb-6 shrink-0">
             ...
             <div className="bg-slate-50 p-3 md:p-4 rounded-xl border border-slate-100 text-center relative group">
                 <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 flex items-center justify-center gap-1 cursor-help">
                   Total Odds
                   <HelpCircle size={10} className="text-slate-400 group-hover:text-indigo-600 transition-colors" />
                 </p>
                 <p className="font-black text-slate-800 text-sm md:text-base">{appData.tierData[tier].totalOdds}</p>
                 
                 {/* Tooltip */}
                 <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 w-48 bg-white border border-slate-200 p-3 rounded-2xl shadow-xl text-[10px] leading-relaxed font-medium text-slate-500 normal-case opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 text-center">
        ...
        <button onClick={() => { handleJoinClick(tier); setShowTierModal(false); }} className="w-full py-4 lg:py-5 rounded-xl md:rounded-2xl font-black uppercase tracking-widest text-xs lg:text-sm shadow-lg transition-all mt-auto bg-slate-900 text-white hover:bg-indigo-900 hover:shadow-xl hover:-translate-y-0.5">
            Select {tier} • ${appData.tierData[tier].price.toLocaleString()}/mo
        </button>
    </div>
  );
      ...
      <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 overflow-y-auto" role="dialog" aria-modal="true">
         ...
         <div className="relative bg-slate-50 w-full max-w-6xl rounded-3xl md:rounded-[3rem] p-5 md:p-10 shadow-2xl animate-in fade-in zoom-in-95 my-8">
            ...
            <div className="md:hidden flex bg-slate-200 p-1 rounded-2xl mb-5 max-w-sm mx-auto">
               {['silver', 'gold', 'diamond'].map((tier) => (
                   <button
                      ...
                      className={`flex-1 py-2.5 text-[10px] font-black uppercase rounded-xl transition-all ${modalPricingTab === tier ? `bg-white shadow-sm ${getTierColor(tier)}` : 'text-slate-500 hover:text-slate-700'}`}
                   >
                      {tier}
                   </button>
               ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
               {['silver', 'gold', 'diamond'].map((tier) => (
                  <div key={tier} className={`${modalPricingTab === tier ? 'flex animate-in fade-in slide-in-from-bottom-2' : 'hidden'} md:flex bg-white rounded-[1.5rem] md:rounded-3xl p-5 md:p-8 border flex-col h-full border-slate-200 relative overflow-hidden shadow-sm`}>
                      <div className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl ${getTierBgGlow(tier)} to-transparent opacity-50 -z-0`}></div>
                      ...
                      <div className="relative z-10">
                          ...
                          <div className="space-y-4 md:space-y-6">
                            <div>
                                ...
                                <p className={`text-2xl md:text-3xl font-black uppercase italic tracking-tighter ${selectedTier === 'silver' ? 'text-slate-300' : selectedTier === 'gold' ? 'text-amber-400' : 'text-indigo-300'}`}>{selectedTier}</p>
                            </div>
                            ...
                            <div className="flex justify-between items-end">
                                ...
                                <div className="text-right">
                                  ...
                                  <p className={`text-lg md:text-xl font-bold ${selectedTier === 'silver' ? 'text-slate-300' : selectedTier === 'gold' ? 'text-amber-400' : 'text-indigo-300'}`}>{appData.tierData[selectedTier].prize}</p>
                                </div>
                            </div>
                            <div className="w-full h-px bg-white/10"></div>
            ...
            <div className="bg-indigo-900 p-6 md:p-10 rounded-3xl md:rounded-[2.5rem] shadow-xl text-white">
               ...
               <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
                 <div>
                   ...
                   <input type="text" className="w-full bg-indigo-800/50 border border-indigo-700 rounded-xl p-3 md:p-4 text-sm md:text-base text-white placeholder-indigo-400 focus:ring-2 focus:ring-amber-400 outline-none" placeholder="Your Name" />
                 </div>
                 <div>
                   <label className="block text-[10px] md:text-xs font-bold uppercase tracking-widest text-indigo-300 mb-2">Email</label>
                   <input type="email" className="w-full bg-indigo-800/50 border border-indigo-700 rounded-xl p-3 md:p-4 text-sm md:text-base text-white placeholder-indigo-400 focus:ring-2 focus:ring-amber-400 outline-none" placeholder="john@example.com" />
                 </div>
                 <div>
                   <label className="block text-[10px] md:text-xs font-bold uppercase tracking-widest text-indigo-300 mb-2">Message</label>
                   <textarea className="w-full bg-indigo-800/50 border border-indigo-700 rounded-xl p-3 md:p-4 text-sm md:text-base text-white placeholder-indigo-400 h-24 md:h-32 focus:ring-2 focus:ring-amber-400 outline-none" placeholder="How can we help?"></textarea>
                 </div>
                 <button className="w-full py-3 md:py-4 bg-amber-400 text-indigo-950 text-sm md:text-base font-black uppercase tracking-widest rounded-xl hover:bg-white transition-colors mt-2">Send Message</button>
               </form>
            </div>
          </div>
        </div>
  ...
  const renderHomePage = () => {
    const primaryFaqs = [
      { q: "What is Amplify?", a: "Amplify is a community-powered giving platform that pools monthly Tzedakah to create greater collective impact..." },
      { q: "Where do the raffle prizes come from?", a: "The prizes are funded from each circle’s pooled donations..." },
      { q: "Why not just give directly?", a: "Direct giving is powerful and encouraged. Amplify exists for those who want their consistent monthly giving to become part of a coordinated collective effort..." },
      { q: "Who selects the charities?", a: "Charities are vetted in advance based on impact and financial transparency..." }
    ];
    const secondaryFaqs = [
      { q: "Is my contribution tax-deductible?", a: "Donations benefiting a 501(c)(3) organization are tax-deductible in the US..." },
      { q: "When am I charged?", a: "Your first contribution is processed immediately upon joining..." },
      { q: "Can I cancel at any time?", a: "Yes. Memberships can be paused or canceled at any time..." },
      { q: "Are the drawings required?", a: "No. Participation in any drawings or appreciation rewards is provided solely as a thank-you..." }
    ];
    ...
    <div className="min-h-screen bg-white font-sans text-slate-900 selection:bg-indigo-100 scroll-smooth relative">
      ...
      {/* Hero Section */}
      <header className="pt-24 pb-12 md:pt-32 md:pb-20 px-4 bg-gradient-to-b from-slate-50 to-white overflow-hidden">
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-12 gap-8 md:gap-12 items-center">
            ...
            <div className="lg:col-span-5 relative mt-4 md:mt-0">
              ...
              <div className="absolute -top-3 -left-2 md:-top-10 md:-left-10 bg-amber-400 p-2.5 md:p-6 rounded-xl md:rounded-[2rem] shadow-xl md:shadow-2xl flex flex-col items-center justify-center border-2 md:border-4 border-white z-20 rotate-[-5deg]">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            <div className="bg-indigo-900/40 p-6 md:p-12 rounded-[2rem] md:rounded-[3rem] border border-white/10 flex flex-row md:flex-col items-center md:items-center text-left md:text-center transition-all duration-300 hover:bg-indigo-900 hover:shadow-[0_15px_40px_-10px_rgba(0,0,0,0.3)] hover:-translate-y-1">
              <div className="bg-white/10 w-14 h-14 md:w-20 md:h-20 rounded-full flex items-center justify-center shrink-0 mr-5 md:mr-0 md:mb-8 text-center"><Users className="text-amber-400 w-6 h-6 md:w-8 md:h-8" /></div>
              <div>
            ...
            <div className="bg-indigo-900/40 p-6 md:p-12 rounded-[2rem] md:rounded-[3rem] border border-white/10 flex flex-row md:flex-col items-center md:items-center text-left md:text-center transition-all duration-300 hover:bg-indigo-900 hover:shadow-[0_15px_40px_-10px_rgba(0,0,0,0.3)] hover:-translate-y-1">
              <div className="bg-white/10 w-14 h-14 md:w-20 md:h-20 rounded-full flex items-center justify-center shrink-0 mr-5 md:mr-0 md:mb-8 text-center"><Sparkles className="text-amber-400 w-6 h-6 md:w-8 md:h-8" /></div>
              <div>
            ...
            <div className="bg-indigo-900/40 p-6 md:p-12 rounded-[2rem] md:rounded-[3rem] border border-white/10 flex flex-row md:flex-col items-center md:items-center text-left md:text-center transition-all duration-300 hover:bg-indigo-900 hover:shadow-[0_15px_40px_-10px_rgba(0,0,0,0.3)] hover:-translate-y-1">
              <div className="bg-white/10 w-14 h-14 md:w-20 md:h-20 rounded-full flex items-center justify-center shrink-0 mr-5 md:mr-0 md:mb-8 text-center"><Trophy className="text-amber-400 w-6 h-6 md:w-8 md:h-8" /></div>
              <div>
                 <h3 className="text-lg md:text-2xl font-bold mb-2 md:mb-4 uppercase tracking-tighter text-white">A Monthly Reward</h3>
                 <p className="text-indigo-100/70 leading-relaxed text-sm md:text-base font-medium">As a thank you for your commitment, you receive exclusive perks and entry into a drawing that triggers as soon as 400 paid members join.</p>
              </div>
            </div>
          </div>
        </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
              ...
              <div className="bg-white p-6 md:p-10 rounded-3xl md:rounded-[2.5rem] shadow-sm md:shadow-xl border border-slate-100 flex flex-col sm:flex-row items-start transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_15px_40px_-10px_rgba(79,70,229,0.15)] hover:border-indigo-200">
                 ...
                 <div>
                    ...
                    <p className="text-sm md:text-base text-slate-600 font-medium leading-relaxed">Life-changing prizes are our way of saying thank you. Providing immense value in appreciation empowers our members to give even more generously.</p>
                 </div>
              </div>
           </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-2 gap-6 md:gap-10 items-stretch">
            ...
            <div className="relative group overflow-hidden rounded-3xl md:rounded-[3rem] shadow-2xl bg-slate-900 min-h-[250px] md:min-h-[400px]">
               ...
               <div className="absolute bottom-4 md:bottom-6 left-1/2 -translate-x-1/2 w-[80%] md:w-[70%] bg-white/95 backdrop-blur-xl py-1 rounded-xl flex items-center justify-center shadow-3xl border border-white/20">
                  <img src="/ChaiLifeline.png" alt="Chai Lifeline Logo" className="max-h-16 md:max-h-28 w-auto object-contain" onError={(e) => { e.currentTarget.style.display='none'; }} />
               </div>
            </div>
          </div>
        </div>
             ...
             <>
                    ...
                    <div className="relative z-10 grid grid-cols-12 gap-8 items-center">
                        <div className="col-span-7">
                            <div className="flex items-center gap-3 mb-2">
                                <div className="bg-amber-400 p-1.5 rounded-lg text-indigo-950"><BarChart3 size={20}/></div>
                        ...
                        <div className="col-span-5 grid grid-cols-3 gap-4">
                            ...
                            <div className="bg-white/5 p-3 rounded-xl border border-white/10 text-center">
                                <p className="text-[9px] font-black uppercase text-amber-400 mb-1">Gold</p>
                                <p className="text-lg font-black tracking-tighter">{appData.communities[activeCommunity]?.gold || 0}</p>
                            </div>
                            <div className="bg-white/5 p-3 rounded-xl border border-white/10 text-center">
                                <p className="text-[9px] font-black uppercase text-indigo-300 mb-1">Diamond</p>
                                <p className="text-lg font-black tracking-tighter">{appData.communities[activeCommunity]?.diamond || 0}</p>
                            </div>
                        </div>
                    </div>
                    <div className="mt-6 pt-6 border-t border-white/10 flex justify-center">
                        <button onClick={() => setShowTierModal(true)} className="px-10 py-3 bg-white text-indigo-900 rounded-xl font-black uppercase tracking-widest text-[10px] hover:bg-amber-400 transition-all flex items-center gap-3 shadow-lg">Join Community <ArrowRight size={14}/></button>
                    </div>
                </div>
                ...
                <div className="md:hidden fixed inset-0 z-[120] flex items-center justify-center p-4 overflow-hidden text-center" role="dialog" aria-modal="true">
                    ...
                    <div className="relative bg-indigo-950 p-6 rounded-3xl text-white animate-in fade-in zoom-in-95 slide-in-from-bottom-6 w-full max-w-sm shadow-2xl border border-white/10">
                        ...
                        <div className="mb-6">
                            <div className="bg-amber-400 w-10 h-10 rounded-xl flex items-center justify-center text-indigo-950 mx-auto mb-3"><BarChart3 size={20}/></div>
                        ...
                        <div className="grid grid-cols-3 gap-2 mb-6">
                            ...
                            <div className="bg-white/5 p-2 rounded-xl border border-white/10">
                                <p className="text-[8px] font-black uppercase text-amber-400 mb-0.5">Gold</p>
                                <p className="text-base font-black">{appData.communities[activeCommunity]?.gold || 0}</p>
                            </div>
                            <div className="bg-white/5 p-2 rounded-xl border border-white/10">
                                <p className="text-[8px] font-black uppercase text-indigo-300 mb-0.5">Diamond</p>
      ...
      <section id="tiers" className="py-16 md:py-24 bg-white px-4 text-center overflow-hidden">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <div className="md:hidden flex bg-slate-200 p-1 rounded-2xl mb-5 max-w-sm mx-auto">
             {['silver', 'gold', 'diamond'].map((tier) => (
                 <button
                    ...
                    className={`flex-1 py-2.5 text-[10px] font-black uppercase rounded-xl transition-all ${mobilePricingTab === tier ? `bg-white shadow-sm ${getTierColor(tier)}` : 'text-slate-500 hover:text-slate-700'}`}
                 >
                    {tier}
                 </button>
             ))}
          ...
          <div className="md:hidden">
            {['silver', 'gold', 'diamond'].map((tier) => (
                <div key={tier} className={`${mobilePricingTab === tier ? 'flex animate-in fade-in slide-in-from-bottom-2 duration-300' : 'hidden'} bg-white rounded-3xl p-5 border border-slate-200 relative overflow-hidden flex-col shadow-sm`}>
                    <div className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl ${getTierBgGlow(tier)} to-transparent opacity-50 -z-0`}></div>
                    {renderTierCardContent(tier)}
                </div>
            ))}
          ...
          <div className="hidden md:grid md:grid-cols-3 gap-6 lg:gap-8 text-left max-w-5xl mx-auto">
            {['silver', 'gold', 'diamond'].map((tier) => (
                <div key={tier} className="bg-white rounded-[2rem] p-6 lg:p-8 border border-slate-200 relative overflow-hidden flex flex-col shadow-sm transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_20px_40px_-15px_rgba(79,70,229,0.15)] hover:border-indigo-200">
                    <div className={`absolute top-0 right-0 w-48 h-48 bg-gradient-to-bl ${getTierBgGlow(tier)} to-transparent opacity-50 -z-0`}></div>
                    {renderTierCardContent(tier)}
                </div>
            ))}
          </div>
  ...
  if (!appData) {
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center font-sans">
            <Rocket className="animate-bounce text-indigo-900 mb-4" size={48} />
            <p className="text-indigo-900 font-bold uppercase tracking-widest animate-pulse text-sm">Loading Platform Data...</p>
        </div>
    );
  }
