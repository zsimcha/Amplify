            ...
            <div className="bg-slate-50 rounded-[3rem] p-10 md:p-16 border border-slate-100 flex flex-col justify-center text-center md:text-left">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-6 justify-center md:justify-start">
                ...
                <div className="flex items-center gap-3 text-red-500">
                    ...
                    <p className="text-xs font-black uppercase tracking-widest leading-none text-slate-700">Impact Goal: $400,000</p>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 md:gap-4 mb-8">
            {Object.keys(communityData).map((name, i) => (
              <button 
                ...
                className={`p-4 md:p-8 rounded-2xl md:rounded-[2rem] border transition-all flex flex-col items-center group ${activeCommunity === name ? 'border-indigo-600 bg-indigo-50 shadow-xl' : 'border-slate-100 bg-white hover:border-indigo-200 hover:shadow-lg'}`}
              >
                ...
                <div className="hidden md:block bg-indigo-950 p-6 rounded-[2.5rem] text-white animate-in fade-in slide-in-from-top-4 relative overflow-hidden shadow-2xl text-left">
                    ...
                    <div className="relative z-10 grid grid-cols-12 gap-8 items-center">
                        <div className="col-span-7">
                            ...
                            <p className="text-indigo-200 text-sm font-medium leading-relaxed max-w-md">Currently generating {communityData[activeCommunity].monthly} in monthly throughput for our partner charities.</p>
                        </div>
                        <div className="col-span-5 grid grid-cols-3 gap-4">
                            <div className="bg-white/5 p-3 rounded-xl border border-white/10 text-center">
                                ...
                                <p className="text-lg font-black tracking-tighter">{communityData[activeCommunity].silver}</p>
                            </div>
                            <div className="bg-white/5 p-3 rounded-xl border border-white/10 text-center">
                                <p className="text-[9px] font-black uppercase text-amber-400 mb-1">Gold</p>
                                <p className="text-lg font-black tracking-tighter">{communityData[activeCommunity].gold}</p>
                            </div>
                            <div className="bg-white/5 p-3 rounded-xl border border-white/10 text-center">
                                <p className="text-[9px] font-black uppercase text-indigo-300 mb-1">Diamond</p>
                                <p className="text-lg font-black tracking-tighter">{communityData[activeCommunity].diamond}</p>
                            </div>
                        </div>
                    </div>
                    <div className="mt-6 pt-6 border-t border-white/10 flex justify-center">
                        <button onClick={() => { setSelectedCommunity(activeCommunity); scrollToSection('tiers'); }} className="px-10 py-3 bg-white text-indigo-900 rounded-xl font-black uppercase tracking-widest text-[10px] hover:bg-amber-400 transition-all flex items-center gap-3 shadow-lg">Join Community <ArrowRight size={14}/></button>
                    </div>
                </div>

                {/* MOBILE VIEW: Pop-up Modal */}
                <div className="md:hidden fixed inset-0 z-[120] flex items-center justify-center p-4 overflow-hidden">
                    <div className="absolute inset-0 bg-indigo-950/80 backdrop-blur-md" onClick={() => setActiveCommunity(null)}></div>
                    <div className="relative bg-indigo-950 p-6 rounded-[2.5rem] text-white animate-in fade-in zoom-in-95 slide-in-from-bottom-6 w-full max-w-sm shadow-2xl border border-white/10">
                        ...
                        <div className="text-center mb-6">
                            ...
                            <h3 className="text-2xl font-black uppercase italic tracking-tighter leading-none">{activeCommunity} Dashboard</h3>
                            <p className="text-indigo-200 text-sm mt-2 leading-snug">Monthly impact: <span className="text-white font-black">{communityData[activeCommunity].monthly}</span></p>
                        </div>
                        ...
                        <button onClick={() => { setSelectedCommunity(activeCommunity); scrollToSection('tiers'); setActiveCommunity(null); }} className="w-full py-4 bg-white text-indigo-900 rounded-2xl font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-3">Join Community <ArrowRight size={14}/></button>
                    </div>
                </div>
             </>
          )}
      ...
      <section id="tiers" className="py-24 bg-slate-50 px-4 text-center">
        <div className="max-w-6xl mx-auto">
            ...
            <table className="w-full text-left border-separate border-spacing-0 rounded-[3rem] overflow-hidden shadow-2xl border border-slate-200 bg-white">
              ...
              <tbody className="bg-white text-center">
                ...
                <tr className="bg-slate-50">
                  ...
                  <td className="p-6 border-r border-slate-200 font-bold text-sm md:text-base">1 / 400</td>
                  <td className="p-6 border-r border-slate-200 font-bold text-sm md:text-base">1 / 400</td>
                  <td className="p-6 font-bold text-sm md:text-base">1 / 400</td>
                </tr>
                <tr>
                  <td className="p-6 font-black border-r border-slate-200 text-slate-700 text-left uppercase text-xs md:text-sm tracking-widest">Other Prizes</td>
                ...
                <tr className="bg-slate-50">
                  <td className="p-6 font-black border-r border-slate-200 text-slate-700 text-left uppercase text-xs md:text-sm tracking-widest">Combined Odds</td>
                  <td className="p-6 font-black border-r border-slate-200 text-black text-sm md:text-lg">{tierData.silver.totalOdds}</td>
                  <td className="p-6 font-black border-r border-slate-200 text-black text-sm md:text-lg">{tierData.gold.totalOdds}</td>
                  <td className="p-6 font-black text-black text-lg md:text-2xl">{tierData.diamond.totalOdds}</td>
                </tr>
                <tr>
                  ...
                  <td className="p-6 md:p-10 border-r border-slate-200 space-y-3 text-xs font-black text-slate-500 uppercase tracking-tighter leading-relaxed text-center">{tierData.silver.perks.map((p, i) => <p key={i}>{p}</p>)}</td>
                  <td className="p-6 md:p-10 border-r border-slate-200 space-y-3 text-xs font-black text-slate-500 uppercase tracking-tighter leading-relaxed text-center">{tierData.gold.perks.map((p, i) => <p key={i}>{p}</p>)}</td>
                  <td className="p-6 md:p-10 space-y-3 text-xs font-black text-indigo-900 uppercase tracking-tighter leading-relaxed text-center">{tierData.diamond.perks.map((p, i) => <p key={i}>{p}</p>)}</td>
                </tr>
          ...
          <div className="mt-16 text-center px-4">
            <p className="text-slate-400 text-[10px] md:text-[11px] font-bold uppercase tracking-[0.3em] max-w-2xl mx-auto leading-relaxed">
