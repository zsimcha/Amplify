...
const App = () => {
  ...
  // Ref to track scroll positions for 'back' navigation memory
        ...
        if (data && data.length > 0) {
          setAppData(prev => {
            ...
            data.forEach(comm => {
              ...
              fetchedData[comm.name] = {
                members: comm.members || 0,
                monthly: comm.monthly || 0,
                silver: comm.silver || 0,
                gold: comm.gold || 0,
                diamond: comm.diamond || 0
              };
            });
            return {
              ...prev,
              allCommunityNames: ["General Circle", ...Array.from(fetchedNames).filter(c => c !== "General Circle").sort()],
              communities: { ...prev.communities, ...fetchedData }
            };
          });
        }
  ...
  // --- ENHANCED ROUTING WITH SCROLL MEMORY ---
  useEffect(() => {
    const handleHashChange = () => {
      const hash = window.location.hash.replace('#', '') || 'home';
      setCurrentView(hash);
      
      // If returning home, restore scroll position. Otherwise jump to top.
      if (hash === 'home' && scrollMemory.current['home'] !== undefined) {
          setTimeout(() => window.scrollTo({ top: scrollMemory.current['home'], behavior: 'auto' }), 10);
      } else {
  ...
  const navigateTo = (view) => {
    ...
    if (currentView === 'home') {
       scrollMemory.current['home'] = window.scrollY;
    }
    ...
    setAppData(prev => ({
      ...
      communities: {
        ...
        [community]: {
          ...
          members: (prev.communities[community]?.members || 0) + 1,
          monthly: (prev.communities[community]?.monthly || 0) + tierPrice,
          [tier]: (prev.communities[community]?.[tier] || 0) + 1
        }
      }
    }));
  ...
  const toggleCommunity = (name) => setActiveCommunity(prev => prev === name ? null : name);
  ...
  const handleCheckoutSubmit = async (e) => {
    ...
    try {
      let communityId;
      const { data: existingCommunity, error: fetchError } = await supabase.from('communities').select('id').eq('name', community).maybeSingle();

      if (existingCommunity) {
        communityId = existingCommunity.id;
      } else {
        const { data: newCommunity, error: insertCommError } = await supabase
          .from('communities')
          .insert([{ name: community, members: 0, monthly: 0, silver: 0, gold: 0, diamond: 0 }])
          .select().single();
      ...
      if (communityId) {
        const { error: subError } = await supabase.from('Subscriptions').insert([{
            ...
            "zip_code": checkoutForm.zipCode, "tier": tier, "community_id": communityId, "status": "pending"
        }]);
        if (subError) console.error("Supabase Insert Error:", subError);
      }
    } catch (err) { console.error("Failed to reach Supabase:", err); }
  ...
  const SecondaryNavbar = () => (
    <nav className="bg-white border-b border-slate-200 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 h-16 md:h-20 flex items-center justify-between">
        ...
        {/* Fixed Spacing Issue here */}
        <button onClick={() => navigateTo('home')} className="text-slate-500 font-bold uppercase tracking-widest text-[10px] md:text-xs flex items-center gap-1 hover:text-indigo-900">
    ...
    const handleCreateCommunity = () => {
       ...
       if (!appData.allCommunityNames.includes(newName)) {
           setAppData(prev => {
               ...
               return { ...prev, allCommunityNames: ["General Circle", ...[...others, newName].sort()], communities: { ...prev.communities, [newName]: { members: 0, monthly: 0, silver: 0, gold: 0, diamond: 0 } } };
           });
       }
        ...
        <div className="max-w-7xl mx-auto px-4 py-8 md:py-20">
          <div className="max-w-5xl mx-auto">
            {signupSuccess ? (
              ...
              // Reordered grid structure to place Summary on top for mobile
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-12">
                
                {/* CHECKOUT FORM - Second on mobile, Left (Col-span-7) on desktop */}
                <div className="order-2 lg:order-1 lg:col-span-7 space-y-6 md:space-y-8">
                  <div className="bg-white p-6 md:p-10 rounded-3xl md:rounded-[2.5rem] shadow-xl border border-slate-100">
                    ...
                    <form onSubmit={handleCheckoutSubmit} className="bg-slate-50 rounded-2xl md:rounded-[2rem] border border-slate-200 shadow-sm p-6 md:p-8 space-y-4">
                        <h3 className="text-lg md:text-xl font-black uppercase tracking-tight text-indigo-950 mb-4 border-b border-slate-200 pb-4">Your Details</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div><label className="block text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">Full Name</label><input required type="text" value={checkoutForm.fullName} onChange={e => setCheckoutForm({...checkoutForm, fullName: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="John Doe" /></div>
                          <div><label className="block text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">Email</label><input required type="email" value={checkoutForm.email} onChange={e => setCheckoutForm({...checkoutForm, email: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="john@example.com" /></div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div><label className="block text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">Phone</label><input required type="tel" value={checkoutForm.phone} onChange={e => setCheckoutForm({...checkoutForm, phone: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="(555) 123-4567" /></div>
                          <div><label className="block text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">Address</label><input required type="text" value={checkoutForm.address} onChange={e => setCheckoutForm({...checkoutForm, address: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="123 Main St" /></div>
                        </div>
                        <div className="grid grid-cols-6 gap-4">
                          <div className="col-span-6 md:col-span-3"><label className="block text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">City</label><input required type="text" value={checkoutForm.city} onChange={e => setCheckoutForm({...checkoutForm, city: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="New York" /></div>
                          <div className="col-span-3 md:col-span-1"><label className="block text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">State</label><input required type="text" value={checkoutForm.state} onChange={e => setCheckoutForm({...checkoutForm, state: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="NY" /></div>
                          <div className="col-span-3 md:col-span-2"><label className="block text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">Zip Code</label><input required type="text" value={checkoutForm.zipCode} onChange={e => setCheckoutForm({...checkoutForm, zipCode: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="10001" /></div>
                        </div>
                        <div className="pt-6 mt-6 border-t border-slate-200">
                          <button type="submit" disabled={isLoading} className="w-full py-4 bg-indigo-900 text-white rounded-xl font-black shadow-lg hover:bg-black transition-all uppercase tracking-widest text-xs md:text-sm flex items-center justify-center gap-2 md:gap-3 disabled:opacity-70 disabled:cursor-not-allowed">
                            {isLoading ? <span className="animate-pulse italic">Processing...</span> : <><Shield size={16} /> Complete Checkout</>}
                          </button>
                        </div>
                    </form>
                ...
                {/* SUMMARY PANEL - First on mobile, Right (Col-span-5) on desktop */}
            ...
            <div className="bg-white p-6 md:p-10 rounded-3xl md:rounded-[2.5rem] shadow-xl border border-slate-100">
              ...
              <div className="space-y-6 md:space-y-8">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div><p className="font-bold text-slate-900 text-base md:text-lg mb-0.5 md:mb-1">Headquarters</p><p className="text-sm md:text-base text-slate-500">New York, NY</p></div>
                </div>
              </div>
            </div>
            <div className="bg-indigo-900 p-6 md:p-10 rounded-3xl md:rounded-[2.5rem] shadow-xl text-white">
               ...
               {contactStatus === 'success' ? (
                 ...
                 <form className="space-y-4" onSubmit={handleContactSubmit}>
                   ...
                   {contactStatus === 'error' && <p className="text-red-400 text-xs text-center font-bold mt-2">There was a problem sending your message. Please try again.</p>}
                 </form>
               )}
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-12 gap-10 md:gap-12 lg:gap-16 items-center">
            <div className="text-left lg:col-span-6">
              ...
              <div className="flex flex-wrap gap-4 text-left">
                <button onClick={() => scrollToSection('tiers')} className="w-full md:w-auto px-8 md:px-12 py-4 md:py-5 bg-indigo-900 text-white rounded-xl md:rounded-2xl font-black text-lg md:text-xl hover:shadow-2xl hover:bg-black transition-all transform hover:-translate-y-1 uppercase tracking-tighter">Join the Circle</button>
              </div>
            </div>
             ...
             <>
                <div className="hidden md:block bg-indigo-950 p-6 rounded-[2.5rem] text-white animate-in fade-in slide-in-from-top-4 relative overflow-hidden shadow-2xl text-left">
                    ...
                    <div className="relative z-10 grid grid-cols-12 gap-8 items-center">
                        <div className="col-span-7">
                            <div className="flex items-center gap-3 mb-2"><div className="bg-[#eab308] p-1.5 rounded-lg text-indigo-950"><BarChart3 size={20}/></div><h3 className="text-2xl font-black uppercase italic tracking-tighter leading-none">{activeCommunity} Dashboard</h3></div>
                        ...
                        <div className="col-span-5 grid grid-cols-3 gap-4">
                            <div className="bg-white/5 p-3 rounded-xl border border-white/10 text-center"><p className="text-[9px] font-black uppercase text-slate-400 mb-1">Silver</p><p className="text-lg font-black tracking-tighter">{appData.communities[activeCommunity]?.silver || 0}</p></div>
                            <div className="bg-white/5 p-3 rounded-xl border border-white/10 text-center"><p className="text-[9px] font-black uppercase text-[#eab308] mb-1">Gold</p><p className="text-lg font-black tracking-tighter">{appData.communities[activeCommunity]?.gold || 0}</p></div>
                            <div className="bg-white/5 p-3 rounded-xl border border-white/10 text-center"><p className="text-[9px] font-black uppercase text-[#818cf8] mb-1">Diamond</p><p className="text-lg font-black tracking-tighter">{appData.communities[activeCommunity]?.diamond || 0}</p></div>
                        </div>
                    </div>
                ...
                <div className="md:hidden fixed inset-0 z-[120] flex items-center justify-center p-4 overflow-hidden text-center" role="dialog" aria-modal="true">
                    ...
                    <div className="relative bg-indigo-950 p-6 rounded-3xl text-white animate-in fade-in zoom-in-95 slide-in-from-bottom-6 w-full max-w-sm shadow-2xl border border-white/10">
                        ...
                        <div className="mb-6"><div className="bg-[#eab308] w-10 h-10 rounded-xl flex items-center justify-center text-indigo-950 mx-auto mb-3"><BarChart3 size={20}/></div><h3 className="text-xl font-black uppercase italic tracking-tighter leading-none">{activeCommunity}</h3><p className="text-indigo-200 text-xs mt-2 leading-snug">Monthly impact: <span className="text-white font-black">{formatCurrency(appData.communities[activeCommunity]?.monthly || 0)}</span></p></div>
                        <div className="grid grid-cols-3 gap-2 mb-6">
                            <div className="bg-white/5 p-2 rounded-xl border border-white/10"><p className="text-[8px] font-black uppercase text-slate-400 mb-0.5">Silver</p><p className="text-base font-black">{appData.communities[activeCommunity]?.silver || 0}</p></div>
                            <div className="bg-white/5 p-2 rounded-xl border border-white/10"><p className="text-[8px] font-black uppercase text-[#eab308] mb-0.5">Gold</p><p className="text-base font-black">{appData.communities[activeCommunity]?.gold || 0}</p></div>
                            <div className="bg-white/5 p-2 rounded-xl border border-white/10"><p className="text-[8px] font-black uppercase text-[#818cf8] mb-0.5">Diamond</p><p className="text-base font-black">{appData.communities[activeCommunity]?.diamond || 0}</p></div>
                        </div>
      ...
      <section className="py-16 md:py-20 bg-indigo-950 text-white px-4 text-center">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="inline-flex flex-col md:flex-row justify-center gap-4 md:gap-8 items-center px-6 md:px-10 py-5 md:py-6 border border-indigo-800 rounded-2xl md:rounded-3xl bg-indigo-900/50 w-full sm:w-auto">
            ...
            <div className="text-center md:text-left"><p className="text-[10px] font-black uppercase tracking-widest text-indigo-400 mb-1">Security</p><p className="font-bold text-sm">Encrypted Transactions</p></div>
          </div>
        </div>
      </section>
