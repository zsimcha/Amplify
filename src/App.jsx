...
const App = () => {
  ...
  const navigateTo = (view) => {
    if (currentView === 'home') scrollMemory.current['home'] = window.scrollY;
    ...
    const handleCreateCommunity = () => {
       ...
       if (!appData.allCommunityNames.includes(newName)) {
           setAppData(prev => {
               ...
               return { 
                 ...prev, 
                 allCommunityNames: ["General Circle", ...sortedNames], 
                ...
                <div className="order-2 lg:order-1 lg:col-span-7 space-y-6 md:space-y-8">
                  <div className="bg-white p-6 md:p-10 rounded-3xl md:rounded-[2.5rem] shadow-xl border border-slate-100">
                    ...
                    <form onSubmit={handleCheckoutSubmit} className="bg-slate-50 rounded-2xl md:rounded-[2rem] border border-slate-200 shadow-sm p-6 md:p-8 space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            <label htmlFor="phone" className="block text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">Phone</label>
                            <input id="phone" name="phone" autoComplete="tel" required type="tel" minLength="14" maxLength="14" value={checkoutForm.phone} onChange={e => setCheckoutForm({...checkoutForm, phone: formatPhoneNumber(e.target.value)})} className="w-full bg-white border border-slate-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="(555) 123-4567" />
                          </div>
                          <div>
                            <label htmlFor="address" className="block text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">Address</label>
                            <input id="address" name="street-address" autoComplete="street-address" required type="text" value={checkoutForm.address} onChange={e => setCheckoutForm({...checkoutForm, address: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="123 Main St" />
                          </div>
                        </div>
                        <div className="grid grid-cols-6 gap-4">
                          <div className="col-span-6 md:col-span-3">
                            <label htmlFor="city" className="block text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">City</label>
                            <input id="city" name="address-level2" autoComplete="address-level2" required type="text" value={checkoutForm.city} onChange={e => setCheckoutForm({...checkoutForm, city: toTitleCase(e.target.value)})} className="w-full bg-white border border-slate-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="New York" />
                          </div>
                          <div className="col-span-3 md:col-span-1">
                            <label htmlFor="state" className="block text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">State</label>
                            <input id="state" name="address-level1" autoComplete="address-level1" required type="text" value={checkoutForm.state} onChange={e => setCheckoutForm({...checkoutForm, state: e.target.value.toUpperCase()})} maxLength="2" className="w-full bg-white border border-slate-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="NY" />
                          </div>
                          <div className="col-span-3 md:col-span-2">
                            <label htmlFor="zip" className="block text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">Zip Code</label>
                            <input id="zip" name="postal-code" autoComplete="postal-code" required type="text" value={checkoutForm.zipCode} onChange={e => setCheckoutForm({...checkoutForm, zipCode: e.target.value.replace(/[^\d]/g, '')})} maxLength="5" className="w-full bg-white border border-slate-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="10001" />
                          </div>
                        </div>
