...
-- REDACTED: the statement Supabase originally recorded here hardcoded a live
-- service_role JWT directly in the trigger definition (as the http_request
-- Authorization header). That trigger was replaced in production by the
-- vault-backed fire_welcome_email_webhook() function added in
-- 20260925000000_secure_welcome_email_webhook.sql, which was never itself
-- captured as a tracked migration until now. The literal key is intentionally
-- omitted from version control; rotate the underlying service_role key if it
-- has not already been rotated, since this value was previously exposed.
